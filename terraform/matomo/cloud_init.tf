module "cloud_init" {
  ...
  docker_compose_content = data.template_file.matomo_service_docker_compose.rendered

  # Azure Login
  azure_identity_login          = true
  azure_user_identity_object_id = azurerm_user_assigned_identity.matomo_vmss_user_identity.id
  acr_login                     = false
  ...
  # FileShare
  file_share_settings             = false
  file_share_storage_account_name = var.matomo_storage_account
  file_share_name                 = local.name
  file_share_mount_path           = local.matomo_data_path
  file_share_gid                  = 33
  file_share_uid                  = 33
