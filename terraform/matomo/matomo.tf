...
module "service_vmss" {
  ...
  # Scaling
  number_of_instances    = var.matomo_min_instances
  vm_type                = var.matomo_vm_type
  overprovision          = var.matomo_scale_overprovision
  priority               = var.matomo_priority
  provision_vm_agent     = true
  single_placement_group = var.single_placement_group
  zone_balance           = var.zone_balance
  zones                  = var.zones
  scale_in_policy        = var.matomo_scale_in_policy
  ...
  extensions = [
    {
      ...
      settings = jsonencode({
        "protocol"    = var.matomo_health_check_protocol
        "requestPath" = var.matomo_health_check_request_path
        "port"        = var.matomo_service_port
      })
    }
  ]
  ...
  identity_ids = [
    azurerm_user_assigned_identity.matomo_vmss_user_identity.id,
  ]
  ...
  depends_on = [
    azurerm_user_assigned_identity.matomo_vmss_user_identity,
    azurerm_role_assignment.matomo_vmss_user_identity_key_vault_secrets_user,
  ]
