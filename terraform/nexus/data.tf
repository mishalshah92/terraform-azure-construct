...
data "azurerm_storage_account" "nexus_storage_account" {
  name                = var.nexus_storage_account
  resource_group_name = var.resource_group
}
...
data "template_file" "nexus_service_docker_compose" {
  template = file("${path.module}/configs/nexus.yml")

  vars = {

    acr_login_server = var.acr_login_server
    nexus_tag        = var.nexus_tag
    nexus_data_path  = local.nexus_data_path
