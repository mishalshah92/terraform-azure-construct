...
module "service_vmss" {
  ...
  name                           = "${local.name}_metabase"
  computer_name_prefix           = local.computer_name_prefix
  location                       = var.location
  terminate_notification_timeout = 0

  # Scaling
  number_of_instances    = var.metabase_min_instances
  vm_type                = var.metabase_vm_type
  overprovision          = var.metabase_scale_overprovision
  priority               = var.metabase_priority
  provision_vm_agent     = true
  single_placement_group = var.single_placement_group
  zone_balance           = var.zone_balance
  zones                  = var.zones
  scale_in_policy        = var.metabase_scale_in_policy
  ...
  extensions = [
    {
      ...
      settings = jsonencode({
        "protocol"    = var.metabase_health_check_protocol
        "requestPath" = var.metabase_health_check_request_path
        "port"        = var.metabase_service_port
      })
    }
  ]
  ...
  identity_ids = [
    azurerm_user_assigned_identity.metabase_vmss_user_identity.id,
  ]
  ...
  depends_on = [
    azurerm_user_assigned_identity.metabase_vmss_user_identity,
    azurerm_role_assignment.metabase_vmss_user_identity_key_vault_secrets_user,
  ]
