module "cloud_init" {
  ...
  docker_compose_content = data.template_file.metabase_service_docker_compose.rendered

  # Azure Login
  azure_identity_login          = true
  azure_user_identity_object_id = azurerm_user_assigned_identity.metabase_vmss_user_identity.id
  acr_login                     = false
  ...
  file_share_storage_account_name = var.metabase_storage_account
  file_share_name                 = local.name
  file_share_mount_path           = local.metabase_data_path
  file_share_gid                  = 0
  file_share_uid                  = 0
