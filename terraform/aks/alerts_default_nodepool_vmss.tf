...
locals {
  vmss_alert_name = "${local.node_group_rg_name}_aks_vmss_${data.azurerm_resources.aks_default_node_pool_vmss.resources[0].name}"
}
...
resource "azurerm_monitor_metric_alert" "vmss_aks_node_pool_cpu_sev2" {
  ...
  scopes = [
    data.azurerm_resources.aks_default_node_pool_vmss.resources[0].id,
  ]
