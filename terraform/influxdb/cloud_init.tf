module "cloud_init" {
  ...
  docker_compose_content = data.template_file.influxdb_service_docker_compose.rendered

  # Azure Login
  azure_identity_login          = true
  azure_user_identity_object_id = azurerm_user_assigned_identity.influxdb_vmss_user_identity.id
  acr_login                     = false
  ...
  file_share_storage_account_name = var.influxdb_storage_account
  file_share_name                 = local.name
  file_share_mount_path           = local.influxdb_data_path
  file_share_gid                  = 1000
  file_share_uid                  = 1000
