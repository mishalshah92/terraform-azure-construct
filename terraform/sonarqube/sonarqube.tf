...
module "service_vmss" {
  ...
  name                           = "${local.name}_sonarqube"
  computer_name_prefix           = local.computer_name_prefix
  location                       = var.location
  terminate_notification_timeout = 0

  # Scaling
  number_of_instances    = var.sonarqube_min_instances
  vm_type                = var.sonarqube_vm_type
  overprovision          = var.sonarqube_scale_overprovision
  priority               = var.sonarqube_priority
  provision_vm_agent     = true
  single_placement_group = var.single_placement_group
  zone_balance           = var.zone_balance
  zones                  = var.zones
  scale_in_policy        = var.sonarqube_scale_in_policy
  ...
  extensions = [
    {
      ...
      settings = jsonencode({
        "protocol"    = var.sonarqube_health_check_protocol
        "requestPath" = var.sonarqube_health_check_request_path
        "port"        = var.sonarqube_service_port
      })
    }
  ]
  ...
  identity_ids = [
    azurerm_user_assigned_identity.sonarqube_vmss_user_identity.id,
  ]
  ...
  depends_on = [
    azurerm_user_assigned_identity.sonarqube_vmss_user_identity,
    azurerm_role_assignment.sonarqube_vmss_user_identity_key_vault_secrets_user,
  ]
