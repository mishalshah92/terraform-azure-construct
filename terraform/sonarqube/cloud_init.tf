module "cloud_init" {
  source = "git::https://github.com/mishalshah92/terraform-azure-core-modules.git//terraform/cloud-init-microservice-fileshare-sonarqube?ref=0.3"

  resource_group_name    = var.resource_group
  docker_compose_content = data.template_file.sonarqube_service_docker_compose.rendered

  # Azure Login
  azure_identity_login          = true
  azure_user_identity_object_id = azurerm_user_assigned_identity.sonarqube_vmss_user_identity.id
  acr_login                     = false
  ...
  file_share_storage_account_name = var.sonarqube_storage_account
  file_share_name                 = local.name
  file_share_mount_path           = local.sonarqube_data_path
  file_share_gid                  = 1000
  file_share_uid                  = 1000
